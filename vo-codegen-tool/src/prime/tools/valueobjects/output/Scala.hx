...
");
	...
	override function writeHeader(file) {
		file.writeString(
	      			...
	      			switch (p.type) {
						...
						default:
							if (!p.isArray() && p.isReference()) try {
								var idType = cast(p.type.getPTypedef().unpackPTypedef(), ClassDef).getIDPropertyFromTWithProperties().type;
