...
(ns prime.vo.util.cassandra-history
  (:require [clojure.test :refer :all]
            ...
            [qbits.alia :as alia]
            [prime.vo.util.cassandra-history :as history]
            [shared.vo]))

(def cluster (delay (alia/cluster "192.168.33.1")))

(defn- column-definitions [vo] (hayt/column-definitions {
...
(defn- prepare-cassandra
  ...
  (alia/with-session (alia/connect cluster)
    (try
      ...
      (catch Exception ex))
    (try
      (alia/execute "CREATE KEYSPACE test1e WITH REPLICATION = {'CLASS' : 'SimpleStrategy', 'replication_factor': 3};")
      (catch Exception ex))
    (try
      (alia/execute "USE test1e;")
      (catch Exception ex))
      (let [
          vo (shared.vo.account/Organization{})
          table (hayt/create-table 
            (keyword (str "t" (Integer/toHexString (.. vo voManifest ID)))) 
...
(deftest insert-functions-test
  (let [
    ...
    vo (shared.vo.account/Organization{:id id :name "test organization" :tags ["awesome" "list" "of" "tags"]})]
    
    (fact "Insert a vo, get it and the same vo should be returned."
      (do 
        (history/put cluster vo nil)
        (history/get cluster (shared.vo.account/Organization{:id id}))) 
      => 
        vo)))
