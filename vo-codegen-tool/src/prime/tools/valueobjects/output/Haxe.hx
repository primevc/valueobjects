...
class Haxe implements CodeGenerator
{
	...
	function addPackage(def:TypeDefinition)
	{
		...
		if (def.module.name.length > 0) {
			...
			a(" import prime.core.traits.IMessagePackable;\n");
			a(" import prime.core.traits.IEditEnabledValueObject;\n");
		}
	}
	...
	private function genClassStart(def:ClassDef, immutable:Bool)
	{
		...
		if (immutable)
			...
			a(" import prime.tools.valueobjects.ObjectChangeSet;\n");
			a(" import prime.tools.valueobjects.ValueObjectBase;\n");
			a(" import prime.tools.valueobjects.xmlmap.XMLString;\n");
			a(" import prime.utils.msgpack.Reader;\n");
			a(" import prime.utils.IfUtil;\n");
			a("  using prime.utils.IfUtil;\n");
			a("  using prime.utils.TypeUtil;\n");
			a("  using prime.utils.NumberUtil;\n");
			a("  using prime.utils.msgpack.Format;\n");
			a("\n\n");
	...
	private function genClassProperties(def:ClassDef, immutable:Bool) //, editable:Bool)
	{
		...
		if (!immutable) {
			...
			if (!def.isMixin && def.superClass == null) {
				...
				a(" implements prime.core.traits.IDuplicatable < " + def.name + "VO >");
			}
		}
		else
		{	
...
//		genFieldSetter(def);
	...
	public function genEnum(def:EnumDef) : Void
	{
		...
		for (e in def.enumerations)
		{
			a("\n\t\t  case "); a(e.name);
			if (e.type != null) {
				a("(_): prime.types.Number.INT_NOT_SET");
			}
			else {
				a(': ' + e.intValue);
			}
...
private class HaxeUtil
{
	public static function getConstructorCall(ptype:PType, bindable:Bool, initializer:String, transient:Bool = false, readonly:Bool = false, sortable:Bool = false)
	{
		switch (ptype) {
			...
			case TuniqueID:				initializer = initializer + ' == null? prime.types.ObjectId.make() : ' + initializer;
			default:
		}
...
*/	
	public static function haxeType(ptype:PType, ?immutableInterface:Bool = false, ?bindable:Bool = false, ?constructorArg = false, ?transient = false, ?readonly = false, ?sortable = false)
	{
		var type = switch (ptype)
		{
			...
			case Tdate:						'Date';
			case Tdatetime:					'Date';
			case Tinterval:					'prime.types.DateInterval';
			case Turi:						'prime.types.URI';
			case Turl: 						'prime.types.URL';
			case TuniqueID:					'prime.types.ObjectId';
			case Temail:					'prime.types.EMail';
			case Tcolor:					constructorArg ? 'Null<prime.types.RGBA>' : 'prime.types.RGBA';
			...
			case Tarray(type, min, max):
				return if (!constructorArg)
				{
					'prime.bindable.collections.' +
				 	...
				 	'prime.utils.FastArray<'+ haxeType(type, true) +'>';
		}
	...
	public static function haxeValueTypeEnum(ptype:PType) return "ValueType." + switch (ptype)
	{
		...
		case Tinterval:					'TClass(prime.types.DateInterval)';
		case Turi:						'TClass(prime.types.URI)';
		case Turl:						'TClass(prime.types.URL)';
		case TuniqueID:					'TClass(prime.types.ObjectId)';
		case Temail:					'TClass(prime.types.EMail)';
		case Tcolor:					'TClass(prime.types.RGBA)';
		case TfileRef:					'TClass(prime.types.FileRef)';
		...
		case Tarray(type, min, max):
		 	'TClass(prime.utils.FastArray<'+ haxeType(type, true) +'>)';
	}
